...
//			this.status.isTermination = false;	
	...
	public boolean markFollowCar(Car frontCar) {
		if (this.status.isTermination) {
			return false;
		}
...
//			}
	...
	public boolean firstTimeMarkFirstCar() {
		int SV1 = this.status.nowSpeed;
		int nowRoadLength = this.ansRoads.get(this.status.nowRoadIndex).length;
		int S1 = nowRoadLength - this.status.location;
		this.status.S1 = S1;
	...
	public void firstTimeMarkFollowCar(Car frontCar) {
		int distance = frontCar.status.location - this.status.location - 1;
		int SV1 = this.status.nowSpeed;
		if (distance < SV1) {
			if (frontCar.status.isTermination) {
...
//			}
	...
	public String toString() {
		...
		if (this.arrivedTime >= 0) {
			return "id:," + this.id + ",isterminal:," + this.status.isTermination + ",nowRoad:,"
					+ this.ansRoads.get(this.status.nowRoadIndex).id + ",Lane:," + this.status.laneNum + ",Location:,"
					+ this.status.location + ",nextRoad:," + "arrived";

		}
		return "id:," + this.id + ",isterminal:," + this.status.isTermination + ",nowRoad:,"
				+ this.ansRoads.get(this.status.nowRoadIndex).id + ",Lane:," + this.status.laneNum + ",Location:,"
				+ this.status.location + ",nextRoad:,";// +this.ansRoads.get(this.status.nextRoadIndex).id;
	}
}
